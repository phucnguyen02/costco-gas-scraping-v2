...
url = "http://localhost:8080/send-gas-prices"
...
def scrape_gas():
    try:
        ...
        # Send warehouse info to the server that handles the chat bot
        warehouse_list = [warehouse.to_dict() for warehouse in db.collection('warehouses').get()]
        headers = {'Content-Type': 'application/json', 'Accept':'application/json'}

        requests.post(url, json = warehouse_list, headers = headers)
